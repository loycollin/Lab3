...
```{r}
HAPPY = replace(HAPPY, HAPPY == "IAP", value = NA)
HAPPY = replace(HAPPY, HAPPY == "DK", value = NA)
HAPPY = replace(HAPPY, HAPPY == "NA", value = NA)
```
...
replace(HAPPY, HAPPY == "89 OR OLDER", 89)
...
levels(reorder(HAPPY$MARITAL, HAPPY$AGE, FUN = mean, na.rm=TRUE))

levels(factor(HAPPY$POLVIEWS, levels = c("EXTREMELY LIBERAL", "LIBERAL", "SLIGHTLY LIBERAL", "MODERATE", "SLIGHTLY CONSERVATIVE", "CONSERVATIVE", "EXTRMLY CONSERVATIVE")))

levels(factor(HAPPY$DEGREE, levels=c("LT HIGH SCHOOL", "HIGH SCHOOL", "JUNIOR COLLEGE", "BACHELOR", "GRADUATE")))
levels(HAPPY$DEGREE) = c("Some HS", "HS", "JC", "BA", "GRADUATE")

levels(factor(HAPPY$FINRELA, levels=c("FAR ABOVE AVERAGE", "ABOVE AVERAGE", "AVERAGE", "BELOW AVERAGE", "FAR BELOW AVERAGE")))
levels(HAPPY$FINRELA) = c("FAR ABOVE", "ABOVE", "AVG", "BELOW", "FAR BELOW")

levels(factor(HAPPY$HEALTH, levels = c("EXCELLENT", "GOOD", "FAIR", "POOR")))

levels(factor(HAPPY$HAPPY, levels = c("NOT TOO HAPPY", "PRETTY HAPPY", "VERY HAPPY")))
...
+ Second Graph (Michelle): 
    + Displays how degree level and financial status relates to happiness. 
    + The people that are least happy have below average financial status.
    + Most people label themselves as "pretty happy"
    + Financial status seems to have a greater impact on happiness.
```{r}
...
ggplot(data = HAPPY) + geom_mosaic(aes(x = product(HAPPY, DEGREE), fill = FINRELA, weight = 1)) + coord_flip()  
